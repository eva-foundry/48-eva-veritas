...
#
# Run:
#   cd C:\AICOE\eva-foundation\48-eva-veritas\.evidence\20260225-sweep4
#   pwsh sweep4.ps1 2>&1 | Out-File sweep4.log -Encoding ASCII
...
Log "=== SWEEP4 START $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ==="
...
foreach ($proj in $projects) {
    ...
    if ($projRecord) {
        ...
        $notesVal = "veritas_scan=2026-02-25 mti=$mti gaps=$gaps stories=$storyCount covered=$covered sweep=sweep4"
...
$results | ConvertTo-Json -Depth 5 | Out-File "$evDir\sweep4-results.json" -Encoding ASCII
Log "Results JSON: $evDir\sweep4-results.json"
...
Log "=== SWEEP4 DONE $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ==="
